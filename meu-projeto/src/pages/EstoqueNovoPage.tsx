...
import { EntradaEstoqueForm } from '../features/estoque/EntradaEstoqueForm';
import { useAuth } from '../hooks/useAuth';

export const EstoqueNovoPage = () => {
  ...
  return (
    <div className="min-h-svh bg-[var(--bg)]">
      ...
      <main className="mx-auto flex max-w-4xl flex-col gap-8 px-6 py-10 sm:px-10">
        <div className="flex items-center justify-between">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-pretty text-sm text-[var(--text)]">
              Registre uma nota de entrada vinculada a um fornecedor.
            </p>
          </div>
          <Link to="/estoque" className="text-sm font-medium text-[var(--accent)] underline underline-offset-2">
            Ver listagem
          </Link>
        </div>
        ...
        <EntradaEstoqueForm
          onSuccess={(nota) => {
            navigate('/estoque', { state: { successMessage: `Nota #${nota.codigo} registrada com sucesso.` } });
          }}
        />
      </main>
    </div>

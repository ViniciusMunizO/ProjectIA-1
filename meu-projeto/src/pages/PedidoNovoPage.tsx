    ...
    <div className="min-h-svh bg-[var(--bg)]">
      ...
      <main className="mx-auto flex max-w-4xl flex-col gap-8 px-6 py-10 sm:px-10">
        <div className="flex items-center justify-between">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-pretty text-sm text-[var(--text)]">
              Selecione o cliente, os produtos e a margem para gerar o documento.
            </p>
          </div>
          <Link to="/pedidos" className="text-sm font-medium text-[var(--accent)] underline underline-offset-2">
            Ver listagem
          </Link>
        </div>
        ...
        <PedidoForm
          onSuccess={(pedido) => {
            navigate('/pedidos', {
              state: { successMessage: `${pedido.tipo === 'PEDIDO' ? 'Pedido' : 'Orçamento'} #${pedido.codigo} registrado com sucesso.` },
            });
          }}
        />
      </main>
